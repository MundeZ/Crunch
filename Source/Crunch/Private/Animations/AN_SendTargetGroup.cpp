...
void UAN_SendTargetGroup::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	...
	for (int i = 1; i < TargetSocketNames.Num(); ++i)
	{
		...
		UKismetSystemLibrary::SphereTraceMultiForObjects(MeshComp, StartLoc, EndLoc, SphereSweepRadius, ObjectTypes,
		                                                 false, ActorsToIgnore, DrawDebugTrace, HitResults, false);
		...
		for (const FHitResult& HitResult : HitResults)
		{
			...
			FGameplayAbilityTargetData_SingleTargetHit* TargetHit = new
